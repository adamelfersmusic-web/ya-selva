...
- Posters/images (`images/*.png`) and their companion markdown docs exist only in the repo, not the vault (except where a vault note explicitly references the artifact, as in `docs/25`). Is that the right split, or should the vault also hold a record of visual artifacts?
